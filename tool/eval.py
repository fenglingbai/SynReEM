...
import numpy as np
from scipy.ndimage import label, zoom, binary_dilation, binary_fill_holes, binary_opening, binary_closing, binary_erosion
...
def calculate_instance(y_true, y_pred):
    ...
    print('Precision: %.2f'%(all_results.precision * 100), 
          ' Recall: %.2f'%(all_results.recall * 100), 
...
def calculate_semantics(y_true, y_pred, resolution):
    ...
    FP = np.sum((gt_array != 1)*(pre_array == 1))
    FN = np.sum((gt_array == 1)*(pre_array != 1))

    Precision = intersection / (intersection + FP)
    Recall = intersection / (intersection + FN)
    
    union = np.sum(pre_array == 1)+np.sum(gt_array == 1)
    ...
    pre_array[pre_array > 0] = 1
    pre_array64 = pre_array.astype(np.uint64)
    pre_array64[pre_array64 == 0] = 0xffffffffffffffff
    pre_cremi_data = Volume(pre_array64, resolution)

    gt_array[gt_array > 0] = 1
    gt_array64 = gt_array.astype(np.uint64)
    gt_array64[gt_array64 == 0] = 0xffffffffffffffff
    gt_cremi_data = Volume(gt_array64, resolution)
    
    # print('compute...')
    clefts_evaluation = Clefts(pre_cremi_data, gt_cremi_data)

    true_positive_count = clefts_evaluation.count_true_positives()
    ...
    Cremi_score = (false_positive_stats['mean']+false_negative_stats['mean'])/2
    Cremi_Precision = true_positive_count / (true_positive_count + false_positive_count)
    Cremi_Recall = true_positive_count / (true_positive_count + false_negative_count)
    Cremi_f1 = 2 * true_positive_count / (2 * true_positive_count + false_positive_count + false_negative_count)

    print('Cremi_score: %.2f'%Cremi_score, ' Cremi_f1: %.2f'%(Cremi_f1 * 100), ' dice: %.2f'%(dice * 100))
